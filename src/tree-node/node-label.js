...
class NodeLabel extends PureComponent {
  ...
  render() {
    ...
    const nodeLabelProps = { className: 'form-check-label form-label' }
    ...
    const className = ['checkbox-item form-check-input', mode === 'simpleSelect' && 'simple-select']
      .filter(Boolean)
      .join(' ')

    return (
      <div className="form-check" title={title || label} htmlFor={id}>
        {mode === 'radioSelect' ? (
        ...
        <label htmlFor={id} {...nodeLabelProps}>
          {label}
        </label>
      </div>
    )
